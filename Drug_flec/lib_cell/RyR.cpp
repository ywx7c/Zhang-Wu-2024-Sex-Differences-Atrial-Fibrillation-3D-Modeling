...
double RyR::Update_RyR_stochastic(double dt, double Caj, double CaSRj) {
  ...
  Po = (RyR_2 + RyR_3) / 100.0;
	...
	double d_flec      = 2.5;                                 //2.5 drug concentration [uM]
	double d_diffusion = 5500.0 / 1000000;
	double k25 = d_flec * d_diffusion;
	double k52 = 0.011;                                       //[ms^-1]
	double k56 = k23; 
	double k36 = d_flec * d_diffusion;
	double k63 = 0.011;
	double k65 = k32;

	int RyR_4 = N_RyR - RyR_1 - RyR_2 - RyR_3 - RyR_6 - RyR_5;

	Update_RyR_rates_stochastic_MN(RyR_1, k12 * dt, k14 * dt);
	int ryr12 = tmp1;
	int ryr14 = tmp2;
	Update_RyR_rates_stochastic_MN_3probs(RyR_2, k23 * dt, k21 * dt, k25 * dt);	
	int ryr23 = tmp1;
	int ryr21 = tmp2;
	int ryr25 = tmp3;
	Update_RyR_rates_stochastic_MN_3probs(RyR_3, k34 * dt, k32 * dt, k36 * dt);	
	int ryr34 = tmp1;
	int ryr32 = tmp2;	
	int ryr36 = tmp3;
	...
	int ryr41 = tmp1;
	int ryr43 = tmp2;
	Update_RyR_rates_stochastic_MN(RyR_5, k52 * dt, k56 * dt);
	int ryr52 = tmp1;
	int ryr56 = tmp2;
	Update_RyR_rates_stochastic_MN(RyR_6, k63 * dt, k65 * dt);
	int ryr63 = tmp1;
	int ryr65 = tmp2;


	RyR_1 = RyR_1 - (ryr12 + ryr14) + (ryr21 + ryr41);
	RyR_2 = RyR_2 - (ryr21 + ryr23 + ryr25) + (ryr12 + ryr32 + ryr52);
	RyR_3 = RyR_3 - (ryr34 + ryr32 + ryr36) + (ryr43 + ryr23 + ryr63);
	RyR_5 = RyR_5 - (ryr52 + ryr56) + (ryr25 + ryr65);
	RyR_6 = RyR_6 - (ryr63 + ryr65) + (ryr36 + ryr56);
	return Po;
}
...
void RyR::Update_RyR_rates_stochastic_MN_3probs(int n, double p1, double p2, double p3)
{
	if (p1+p2+p3 >= 1){
		std::cout << "Need smaller dt Update_RyR_rates_stochastic_MN()!!!! "<< std::endl;
		std::exit(0);

...
void CSubcell::init(double initci, double initcj, double initnai, int flag_AF, int flag_female)
{
        AF      = flag_AF;     // 0: nSR; 1: cAF
        female  = flag_female; // 0: male; 1: female
        ISO     = 0; 


        gleak   = 1.035 * 0.00001;
        gCa_iso_af_scale = 1.475 * (1 - 0.5*AF*(1 - female));
        gca     = gCa_iso_af_scale * (1 - 0.0845);            // Flecainide
        pNa     = 1.0 / 8113.0 * 0.75e-8 * 0.96245 * (1e-10); // [cm/sec*F]
        pK      = 1.0 / 8113.0 * 1.35e-7 * 0.96245 * (1e-10); // [cm/sec*F]
        vup     = 1.2 * 0.3;          // [uM/ms] 
        kup     = 2.5 * 0.123 * 2.0 * (1 - 0.1*AF*(1-female));
        ...
        if(flag_female == 0 && flag_AF == 1)
        {
                ryr_map_file = "pool_ryr/male_cAF.txt";
                csq_map_file = "pool_csq/male_cAF.txt";
        }else if(flag_female == 1 && flag_AF == 0)
        {
                ryr_map_file = "pool_ryr/female_nSR.txt";
                csq_map_file = "pool_csq/female_nSR.txt";
        }else if(flag_female == 1 && flag_AF == 1)
        {
                ryr_map_file = "pool_ryr/female_cAF.txt";
                csq_map_file = "pool_csq/female_cAF.txt";
        }else
        {
                ryr_map_file = "pool_ryr/male_nSR.txt";
                csq_map_file = "pool_csq/male_nSR.txt";
        }

        // To load RyR pattern
        std::ifstream inRyR(ryr_map_file.c_str());
        if(inRyR.fail()){
                cout << "RyR file not exist" << endl;
                exit(1);
        }
        std::string RyRline;
        int RyR_id   = 0;
        int RyR_value = 0;
        while (std::getline(inRyR, RyRline))
        {
                // each CRU at least has 1 RyR
                std::stringstream ssRyR(RyRline);
                ssRyR >> RyR_value;
                set_RyR(RyR_id, RyR_value);
                ++RyR_id;
        }

        // To load CSQ pattern
        std::ifstream inCSQ(csq_map_file.c_str());
        if(inCSQ.fail()){
                cout << "CSQ file not exist" << endl;
                exit(1);
        }
        std::string CSQline;
        int CSQ_id   = 0;
        int CSQ_value = 0;
        while (std::getline(inCSQ, CSQline))
        {
                // each CRU at least has 1 CSQ
                std::stringstream ssCSQ(CSQline);
                ssCSQ >> CSQ_value;
                set_CSQ(CSQ_id, CSQ_value);
                ++CSQ_id;
        }
        ...
        for (int i = 0; i < n; ++i)
        {
                LTCC_Markov_vec.push_back( LTCC_Markov(map_nlcc[i], i+1000*seed, AF, female) );
                random_num.push_back(xor_rand(seed, i+1000*seed));
                RyR_vec.push_back(RyR(map_nryr[i], map_csq[i], i+1000*seed, AF, female));
        }
...
void CSubcell::delarray(void)
{
        ...
        delete [] JSR_buffer_factors;
        delete [] I_ClCa_sl_array ;
        delete [] I_nak_sl_array  ;
        delete [] I_nabk_sl_array ;
        delete [] I_Na_sl_array   ;
...
void CSubcell::set_CSQ(int id, double csq_bmax) 
{
        map_csq[id]    = csq_bmax;
}
